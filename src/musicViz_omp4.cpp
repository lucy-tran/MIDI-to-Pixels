...
int main(int argc, char** argv) {
   ...
   if (argc == 0) {
      ...
      getFileAndNumThreads(argc, argv, &fileName, &nThreads);
   ...
   private(event, mev) reduction(max:maxWidth) default(none) 
   {
      ...
      for (event = eventItr; event<numEvents; event+=numThreads) {
         ...
         if (mev->isNoteOn()) {
            ...
            while (y<9 && (colorMap[channelId][y][xStart].R!=0 || 
                  colorMap[channelId][y][xStart].G!=0 || 
                  colorMap[channelId][y][xStart].B!=0)) { //the starting cell is painted 
               y++;
               #pragma omp critical
   ...
   prtRgb(channelSound.size(), maxWidth, fileDurationInTicks, &colorMap[0][0][0], "rgb.dat");
