...
int main(int argc, char** argv) {
   ...
   int numEventsRemain = numEvents - eventItr;
   int chunkSize = round(numEventsRemain / numThreads);      // find chunk size

   //Start timing
   double start = omp_get_wtime();

   #pragma omp parallel shared(numEvents, numThreads, chunkSize, eventItr, channelIdx, channelSound, channelWidths, midifile, colorMap, scale) \
   private(id, event, mev, channel, channelId, xStart, xEnd, y) reduction(max:maxWidth) default(none) num_threads(numThreads)
   {
      id = omp_get_thread_num();
      int startPt = eventItr + id * chunkSize;               // find starting index
      int stopPt = startPt + chunkSize;             // find stopping index
      for (event = startPt; event<stopPt; event++) {
